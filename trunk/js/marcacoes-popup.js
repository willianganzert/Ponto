...
function gravaMarcacoes(horarios){
	for(var i = 0;i<horarios.length;i++){
		html5rocks.webdb.insertMarca({user:currentSettings.username,dataHora:horarios[i].getTime()});
	}
	setTimeout(carregaMarcacoes,1000);
}
...
function testeMensagem(msg){
    sendMessage({type:"teste",msg:msg},testeRetornoMensagem);
}
function testeRetornoMensagem(msgret){alert(msgret)}